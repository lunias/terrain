...
public class NotTurningOffItemIntent extends Intent {
    ...
    private static final Pattern PATTERN =
            Pattern.compile("(?:my\\s)?(.*?)\\s(.*\\s)?(?:is\\s)?.*(?:not|isn'?t|don'?t|doesn'?t|(?:does\\snot)|won'?t|(?:will\\snot))\\s(?:(?:turn|go|power)ing\\s(?:off|down)|(?:turns|turn|goes|go|powers|power)\\s(?:off|down))");
